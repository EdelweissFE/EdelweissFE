...
Fri Oct 6 2018:
    ...
    This version (mk1) directly accesses the underlying BftUel, and thus 
    it is able to completely release the GIL throughout the complete prange loop.
    Naturally, it is compatible only with cython elements based on BftUels.
    However, it seems that there is no measurable performance advantage over mk2, which
    is not dependent on a  underlying BftUel (and is thus also compatible with python or
    cython elements). 

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 24 19:35:09 2017

@author: matthias
"""
from fe.utils.misc import stringDict
from collections import defaultdict

def generateEnsightOptions(actionDefinitionLines, jobInfo, modelInfo, time, 
                                                               stepActions, 
                                                               U, P):
    oldOptions = stepActions.get('EnsightOptions', defaultdict(dict))
    for line in actionDefinitionLines:
        
        newOptions = stringDict(line)
        targetManager = newOptions.get('managerName', 'defaultName')
        oldOptions[targetManager].update(newOptions)
        
    return oldOptions

        